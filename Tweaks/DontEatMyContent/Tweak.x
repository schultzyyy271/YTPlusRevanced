...
- (void)notifyEngagementPanelContainerControllerWillHideFinalPanel {
    ...
    if (![self isPeekingSupported] && !isEngagementPanelViewControllerRemoved) {
        isEngagementPanelVisible = NO;
        BOOL isLandscape = NO;
        if ([self respondsToSelector:@selector(isWatchLandscapeEngagementPanel)])
            isLandscape = ((BOOL (*)(id, SEL))objc_msgSend)(self, @selector(isWatchLandscapeEngagementPanel));
        else if ([self respondsToSelector:@selector(landscapeEngagementPanelEnabled)])
            isLandscape = ((BOOL (*)(id, SEL))objc_msgSend)(self, @selector(landscapeEngagementPanelEnabled));
        if (isLandscape && !isZoomedToFill) {
            DEMC_activateConstraints();
        }
    }
    %orig;
