...
@implementation YTPColorSwatch
// Plain UIButton subclass acting as a color swatch. No HBColorWell needed.
@end

@implementation SponsorBlockTableCell
// UIColorPickerViewControllerDelegate (replaces HBColorPickerDelegate — no libcolorpicker needed)
- (void)colorPickerViewControllerDidSelectColor:(UIColorPickerViewController *)viewController {
    UIColor *color = viewController.selectedColor;
    self.colorWell.color = color;
    // Update the swatch tint to match
    [self.colorWell setBackgroundColor:color];
...
- (void)presentColorPicker:(UITableViewCell *)sender {
    // Native UIColorPickerViewController — no Alderis/libcolorpicker dependency
    UIColorPickerViewController *viewController = [[UIColorPickerViewController alloc] init];
    viewController.delegate = self;
    viewController.selectedColor = self.colorWell.color ?: [UIColor whiteColor];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section <= 7) {
        ...
        if (indexPath.row == 0) {
            ...
            // Native color swatch button — no HBColorWell needed
            YTPColorSwatch *colorWell = [[YTPColorSwatch alloc] initWithFrame:CGRectMake(0,0,32,32)];
            colorWell.layer.cornerRadius = 8;
            colorWell.layer.masksToBounds = YES;
            colorWell.layer.borderWidth = 1.0;
            colorWell.layer.borderColor = [UIColor separatorColor].CGColor;
            UIColor *color = colorWithHexString([categorySettings objectForKey:[NSString stringWithFormat:@"%@Color", tableCell.category]]);
            colorWell.color = color ?: [UIColor whiteColor];
            [colorWell setBackgroundColor:colorWell.color];
