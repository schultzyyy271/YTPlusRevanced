// YouTubeHeader/ pkg is not present in this build environment.
// All the YT classes used below are declared in YTVideoOverlay_Compat.h
// (included via Header.h -> Init.h).
#import "Header.h"
#import "Init.h"
...
// YTVideoOverlay's own settings section is disabled — all tweak toggles
// are handled by Settings.x under YouTube Plus Revanced > Tweaks.
// The %group is still initialized so the registerTweak:metadata: and
// updateYTVideoOverlaySectionWithEntry: methods are installed.

%hook YTSettingsGroupData

- (NSArray <NSNumber *> *)orderedCategories {
    // Don't inject YTVideoOverlaySection into the top-level settings list
    return %orig;
}
...
+ (NSArray <NSNumber *> *)settingsCategoryOrder {
    // Don't inject YTVideoOverlaySection into the settings order
    return %orig;
}
