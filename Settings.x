...
- (void)updateYTPlusSectionWithEntry:(id)entry {
    ...
    // ── Version Spoofer ───────────────────────────────────────────────────────
    NSArray *versionList = ytpSpoofVersionList();   // single source of truth (YTPlus.h)

    YTSettingsSectionItem *versionSpooferToggle = [item switchItemWithTitle:@"Version Spoofer"
        titleDescription:@"Spoof the app version reported to YouTube. Useful if an update breaks features. Fully restart YouTube after changing this for it to take effect."
        accessibilityIdentifier:@"YTPlusSectionItem"
    ...
    YTSettingsSectionItem *versionSpooferPicker = [item itemWithTitle:@"Spoofed Version"
        ...
        detailTextBlock:^NSString *{
            ...
            NSString *cur = ytpString(@"versionSpooferValue");
            return cur.length ? cur : @"Not set";
        }
        selectBlock:^BOOL(YTSettingsCell *cell, NSUInteger arg1) {
            if (!ytpBool(@"versionSpooferEnabled")) return NO;
            NSMutableArray *rows = [NSMutableArray array];
            for (NSString *ver in versionList) {
                [rows addObject:[item checkmarkItemWithTitle:ver titleDescription:nil selectBlock:^BOOL(YTSettingsCell *c, NSUInteger idx) {
                    ytpSetString(ver, @"versionSpooferValue");   // store the string itself
                    [settingsVC reloadData];
                    return YES;
                }]];
            }
            NSString *cur = ytpString(@"versionSpooferValue");
            NSUInteger sel = cur.length ? [versionList indexOfObject:cur] : NSNotFound;
            YTSettingsPickerViewController *picker = [[%c(YTSettingsPickerViewController) alloc]
                initWithNavTitle:@"Spoofed Version" pickerSectionTitle:nil rows:rows
                selectedItemIndex:sel parentResponder:[self parentResponder]];
            [settingsVC pushViewController:picker];
            return YES;
        }];
    [sectionItems addObject:versionSpooferPicker];

    // Live proof: reads exactly what +[YTVersionUtils appVersion] returns right now
    // (the same value embedded in YouTube's API requests). Non-tappable status row.
    ...
    [sectionItems addObject:versionSpooferLive];
